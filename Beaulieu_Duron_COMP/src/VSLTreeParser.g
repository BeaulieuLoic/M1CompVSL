...
: ^(ASSIGN_KW e=expression[symTab] (
      (i1=IDENT {code = Code3aGenerator.genAff($ASSIGN_KW,$i1.text,e,symTab);})// affectation variable
      |(^(ARELEM  i2=IDENT exp=expression[symTab]))// affectation tableau
        {
...
expression [SymbolTable symTab] returns [ExpAttribute expAtt]
  ...
  |^(MUL e1=expression[symTab] e2=expression[symTab]) //operation multiplier
    { 
;
  ...
  | ^(DIV e1=expression[symTab] e2=expression[symTab]) //operation diviser
    { 
...
primary [SymbolTable symTab] returns [ExpAttribute expAtt]
  ...
  | ^(NEGAT e2=primary[symTab]) //en cas de signe '-'e2 on appelle l'operation SUB, soit (0-e2)
    {
